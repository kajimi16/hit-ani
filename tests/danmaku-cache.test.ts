...
test("写入后可读回，count 与 total 都保留", async () => {
  ...
  await writeDanmakuCache("TestService", 1001, { items, total: 4909 });

  const cached = await readDanmakuCache("TestService", 1001, 60_000);
...
test("未写入的 key 返回 null（调用方应回源）", async () => {
  await clearDanmakuCache();
  assert.equal(await readDanmakuCache("TestService", 99999, 60_000), null);
});

test("过期条目视为未命中，且被顺带清理", async () => {
  await clearDanmakuCache();
  await writeDanmakuCache("TestService", 1002, { items: makeItems(5), total: 5 });

  // TTL 设为 0，立刻过期
  assert.equal(await readDanmakuCache("TestService", 1002, 0), null);

  // 惰性删除应已把它清掉
  const stats = await danmakuCacheStats();
  assert.equal(stats.episodes, 0, "过期条目应被删除，而不是留在表里");
});

test("重复写入同一 key 是 upsert（不产生重复行）", async () => {
  await clearDanmakuCache();
  await writeDanmakuCache("TestService", 1003, { items: makeItems(10), total: 10 });
  await writeDanmakuCache("TestService", 1003, { items: makeItems(20), total: 20 });

  const cached = await readDanmakuCache("TestService", 1003, 60_000);
  assert.equal(cached?.items.length, 20, "应是覆盖而非追加");
  assert.equal((await danmakuCacheStats()).episodes, 1);
...
test("不同 service + episodeId 相互隔离", async () => {
  await clearDanmakuCache();
  await writeDanmakuCache("SvcA", 2001, { items: makeItems(3), total: 3 });
  await writeDanmakuCache("SvcB", 2001, { items: makeItems(7), total: 7 });

  assert.equal((await readDanmakuCache("SvcA", 2001, 60_000))?.items.length, 3);
  assert.equal((await readDanmakuCache("SvcB", 2001, 60_000))?.items.length, 7);
...
test("空结果也缓存（避免反复回源确认「确实没有」）", async () => {
  await clearDanmakuCache();
  await writeDanmakuCache("TestService", 1004, { items: [], total: 0 });

  const cached = await readDanmakuCache("TestService", 1004, 60_000);
...
test("pruneDanmakuCache 清理超过指定年龄的条目", async () => {
  await clearDanmakuCache();
  await writeDanmakuCache("TestService", 1005, { items: makeItems(2), total: 2 });
  ...
  // 保留窗口很大 → 不该删任何东西
  await writeDanmakuCache("TestService", 1006, { items: makeItems(2), total: 2 });
...
test("danmakuCacheStats 报告条数与字节数", async () => {
  await clearDanmakuCache();
  await writeDanmakuCache("TestService", 1007, { items: makeItems(500), total: 500 });

  const stats = await danmakuCacheStats();
  assert.equal(stats.episodes, 1);
  assert.ok(stats.totalBytes > 0, "应报告压缩后的字节数");
  console.log(`     500 条压缩后 ${(stats.totalBytes / 1024).toFixed(1)} KB`);
