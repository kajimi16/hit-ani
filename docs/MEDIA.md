...
| | 外链跳转（**本项目采用的方案**） | 嵌入播放（**已否决，不实现**） |
| --- | --- | --- |
| 防盗链 Referer | **无影响** —— 用户跳到对方页面，由其自行处理 | 需要伪造请求头 |
| CORS | **无影响** | 需对方授权跨域，普通站点不会给 |
| 带宽 | **零** —— 字节流不经我们服务器 | 全部经过服务器（校内带宽无法承受） |
| 平台角色 | 索引 / 播放器 | 内容分发方 |

**「点击即看」在外链模式下完全可行，且不需要任何代理** ——
只要源指向的是**有自己播放器的网站**。磁力链接是唯一例外：
浏览器点它只会唤起 BT 客户端。

> 说明：上表右列只是**对照分析**，用于解释为什么选左列。
> 本项目不实现、也不建议实现嵌入播放 —— 那会让平台从索引方变成分发方。
